...
export default async function handler(
  ...
  await Promise.all(
    order.orderItems.map(async (item) => {
      let orderItemId = await database
        .query(
      ...
      await Promise.all(
        item.products.map((product) => {
          return Promise.all([
            ...
            database.query(
              `update product set quantity_in_stock = quantity_in_stock - ${item.quantity} where product_id = ${product.id} AND quantity_in_stock >= 0`
            ),
          ]);
        })
      );
